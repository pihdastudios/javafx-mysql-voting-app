...
public class MySqlCon {
    ...
    public static void addVote(String pNIM, int pCand) throws SQLException {
        ...
        String query = " insert into voters (nim, candidate, date_created)"
                + " values (?, ?, ?)";
    ...
    public static void backupDbtoSql(String pDir) {
        try {
            ...
            String executeCmd = "mysqldump -u" + username + " -p" + password + " --database " + dbName + " -r " + savePath;
    ...
    public static void startConnection() {
        if (StringUtils.isNotEmpty(username) || StringUtils.isNotEmpty(password) || StringUtils.isNotEmpty(address) || StringUtils.isNotEmpty(dbName)) {
            try {
                conn =
                        DriverManager.getConnection("jdbc:mysql://" + address + "/" + dbName,
                                username, password);
            } catch (SQLException ex) {
                // handle any errors
                System.out.println("SQLException: " + ex.getMessage());
